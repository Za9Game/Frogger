...
int startGame(WINDOW* w){
    ...
    while(true){
        ...
        pthread_create(&thread_id, NULL, &player, NULL); // creo il thread del player
        ...
        pthread_create(&thread_id2, NULL, &spawnCrocodile, NULL); // creo il thread del cocodrillo
        
        gi.time = TIME_AMOUNT;
        prev_score = gi.score;
        status = render(w, &gi, client_socket); // status rappresenta come si è conclusa la partita (rana morta oppure passato una porta)
            
        pthread_cancel(thread_id); // uccido i thread
