...
void kill_Childs(pid_t pids[], int pid_index){
...
void initialize_sprite_crocodiles(crocodile_info crocodiles[]){
    for(int i=0;i<NUM_CROCODILE;i++){
        ...
        crocodiles[i].direction_changed = false;
    }
}
...
void initializeBushes(int pipe_fds[], pos bushes[], pid_t pids[], int *pid_index){
    pid_t pid;
    for(int i=0;i<NUM_BUSHES;i++){
        do{
        ...
        if(pid < 0) { perror("fork call"); _exit(2);}  // Check fork a buon fine
            ...
            bush(pipe_fds, i); // spawno il nuovo cespuglio
            _exit(0);
        }
        
        pids[*pid_index] = pid;
        *pid_index+=1;
        if(*pid_index >= NUM_PIDS)
            *pid_index=0;
    }
}
...
int render(int pipe_fds[], WINDOW* w, game_info* gi, int client_socket){
    pid_t pids[NUM_PIDS];
    int pid_index=0;
    ...
    initializeBushes(pipe_fds, bushes, pids, &pid_index);

    pos tane[NUMBER_DOORS]; // tane
    initialize_tane(tane, (*gi).doors);

    pid=fork(); // CREAZIONE TIMER
    if(pid < 0) { perror("fork call"); _exit(2);}  // Check fork a buon fine
    else if (pid == 0) {
        timer(pipe_fds);
        _exit(0);
    ...
    while(true){
        ...
        if(&m!=(msg*){0}){       
            if(a<=0){
                read(pipe_fds[0], &m, sizeof(m));
            }
            if(m.t == 'P'){ // ha ricevuto un pid di un cocodrillo
                first_crocodile=true;
                pids[pid_index] = m.m;
                pid_index++;
                if(pid_index >= NUM_PIDS)
                    pid_index=0;
            ...
            else if(m.t=='c' && first_crocodile){ //messaggio da cocodrillo
                ...
                if(crocodiles[m.m].direction == 0){
                    crocodiles[m.m].position.x += m.velocity;
                }
                else{
                    crocodiles[m.m].position.x -= m.velocity;
                }
                //crocodiles[m.m].position = m.position;
                if(crocodiles[m.m].position.y == HEIGHT-4-(FLUSSI/2)){
                    if(m.direzione == 0 && crocodiles[m.m].position.x >= WIDTH/2){
                        crocodiles[m.m].direction = 1;
                        crocodiles[m.m].direction_changed = true;
                    }
                    else if(m.direzione == 1 && crocodiles[m.m].position.x <= WIDTH/2){
                        crocodiles[m.m].direction = 0;
                        crocodiles[m.m].direction_changed = true;
                    }
                }
                if(m.spawned){
                    crocodiles[m.m].direction_changed = false;
                    crocodiles[m.m].direction = m.direzione;
                    crocodiles[m.m].velocity = m.velocity;
                    crocodiles[m.m].bad_crocodile = m.bad_crocodile;
                    crocodiles[m.m].position = m.position;
                }/*
                if(crocodiles[m.m].direction_changed = true){
                    if(m.direzione == 0){
                        m.position.x
                    }
                ...
                int collision = collisionBullets(pipe_fds, bullets, player_pos, crocodiles, bushes, pids, &pid_index); 
                if(collision == -2){ // è uguale a zeor quando il priettile colpisce il player
                    kill_Childs(pids, pid_index);
                    return 0;
                }
